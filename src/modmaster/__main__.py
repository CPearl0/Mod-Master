...
from modmaster.config import config_manager

manager = config_manager()
config_exist = manager.config_exists()
config = manager.load_config()

langsmith_api_key = config.get("langchain", "api-key")
api_key = config.get("zhipu", "api-key")
model = config.get("zhipu", "model")

if (not config_exist or api_key == "输入智谱API Key"):
